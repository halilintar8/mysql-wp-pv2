        ...
        stage('Deploy to Kubernetes') {
            steps {                
                container('kubectl') {
                    ...
                    script {
                        step([$class: 'KubernetesDeploy', authMethod: 'certs', apiServerUrl: 'https://10.8.1.120:6443', credentialsId: 'k8sCertAuth', config: '$(ls *.yaml | awk ' { print " -f " $1 } ')', variableState: 'ORIGIN_REPO,REPO,IMAGE_TAG4'])
